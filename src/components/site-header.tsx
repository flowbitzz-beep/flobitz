...
export function SiteHeader() {
  ...
  // Close mobile menu on path or hash change
  useEffect(() => {
    setIsOpen(false);
  }, [pathname, hash]);
    ...
    <>
      ...
      >
        <div className="mx-auto flex h-16 max-w-7xl items-center justify-between px-5 sm:h-[4.25rem] sm:px-8">
          <Link
            href="/"
            className="group flex z-50 shrink-0 items-center gap-3 outline-none ring-indigo-400/40 focus-visible:ring-2 focus-visible:ring-offset-2 focus-visible:ring-offset-white sm:gap-3.5"
          ...
          >
            <Image
              ...
              priority
            />
            <span className="text-[1.05rem] font-semibold tracking-tight text-zinc-900 sm:text-lg uppercase font-sans">
              Flobitz
            </span>
          </Link>

          {/* Desktop Nav */}
          <nav className="hidden items-center gap-4 sm:gap-8 lg:gap-10 md:flex">
            {navItems.map((item) => {
                ...
                <div key={item.href} className="relative flex items-center h-full">
                  <Link
                    ...
                    className={[
                      ...
                      active ? "text-[#0f0e47] font-bold" : "text-zinc-500 hover:text-[#0f0e47]",
                    ].join(" ")}
                  >
                    {item.label}
                  </Link>
          ...
          <button
            ...
            className="flex z-50 h-10 w-10 items-center justify-center rounded-xl bg-[#0f0e47]/5 text-[#0f0e47] transition-all hover:bg-[#0f0e47]/10 md:hidden"
      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div
            ...
            className="fixed inset-0 z-[55] bg-white/80 backdrop-blur-2xl md:hidden"
          >
            <nav className="flex h-full flex-col items-center justify-center gap-8 px-5">
                  ...
                  >
                    <Link
                      ...
                      className={[
                        ...
                        active ? "text-[#0f0e47]" : "text-zinc-400 hover:text-[#0f0e47]",
                      ].join(" ")}
                    >
                      {item.label}
                    </Link>
